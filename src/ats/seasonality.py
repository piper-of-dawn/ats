...
def fit_seasonal_ols(monthly_returns: pl.DataFrame) -> sm.regression.linear_model.RegressionResultsWrapper:
    target = monthly_returns["LogReturn"].to_numpy()
    features = monthly_returns.select(MONTH_COLUMNS).to_numpy().astype(float)
    return sm.OLS(target, features).fit(cov_type="HC3")


def extract_result(ticker: str, model: sm.regression.linear_model.RegressionResultsWrapper) -> SeasonalityResult:
...
def fit_seasonal_quantile(monthly_returns: pl.DataFrame) -> sm.regression.linear_model.RegressionResultsWrapper:
    target = monthly_returns["LogReturn"].to_numpy()
    features = monthly_returns.select(MONTH_COLUMNS).to_numpy().astype(float)
    return sm.QuantReg(target, features).fit(q=0.5)
